...
from typing import Iterable, List, Literal, Optional, TypeAlias, Union

from litellm import ChatCompletionRedactedThinkingBlock, ChatCompletionThinkingBlock
...
class ChatCompletionAssistantMessageParamWrapper(TypedDict, total=False):
    ...
    """The thinking blocks generated by the model, such as reasoning blocks.
    
    Thinking blocks are a LiteLLM property for reasoning content: 
    - https://docs.litellm.ai/docs/reasoning_content

    Each provider implements its own format for reasoning traces and signatures, LiteLLM
    normalizes them into a common format for us to use. These thinking blocks must be passed
    along from one turn to another to allow the model to continue its reasoning.
    """
