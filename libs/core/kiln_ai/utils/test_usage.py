"""Tests for the Usage / MessageUsage models.

Covers the MessageUsage / Usage split, the ``__add__`` semantics on both
classes, and ``MessageUsage.from_trace``.

The back-compat surface of ``kiln_ai.datamodel.usage`` (which re-exports
these classes) is covered in ``datamodel/test_usage.py``. Field-level
validation tests live in ``datamodel/test_example_models.py``.
"""

import pytest

from kiln_ai.utils.open_ai_types import ChatCompletionMessageParam
from kiln_ai.utils.usage import MessageUsage, Usage
