"""Token usage and cost models.

These live in ``kiln_ai.utils`` rather than ``kiln_ai.datamodel`` to break an
import cycle, and this docstring is the single write-up of it.

``kiln_ai.utils.open_ai_types`` needs ``MessageUsage`` as a real class at
import time: it builds a pydantic ``TypeAdapter`` at module scope, which
resolves its TypedDict annotations eagerly, so a ``TYPE_CHECKING``-only
import is not enough. While these classes lived in ``kiln_ai.datamodel.usage``,
reaching them ran the whole ``kiln_ai.datamodel`` package init, which chains
down to ``datamodel/task_run.py``, which imports ``open_ai_types`` back --
raising ImportError whenever ``open_ai_types`` was the first kiln module
imported.

The layering rule that keeps the cycle broken: *importing this module must
not pull in ``kiln_ai.datamodel``*. That binds more than the import list
below -- ``kiln_ai/utils/__init__.py`` runs first and eagerly imports
``config``, ``formatting`` and ``lock``, so the rule covers the package init
and anything it reaches too. ``utils/test_import_layering.py`` enforces the
real rule rather than this file's imports: it imports this module first in a
fresh interpreter and asserts ``kiln_ai.datamodel`` never lands in
``sys.modules``.

``kiln_ai.datamodel.usage`` re-exports both classes, so external callers and
any serialized reference to the old path keep working.
"""
