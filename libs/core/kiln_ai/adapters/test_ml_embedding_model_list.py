...
class TestKilnEmbeddingModelFamily:
    """Test cases for KilnEmbeddingModelFamily enum"""

    def test_enum_values(self):
        """Test that enum values are correctly defined"""
        assert KilnEmbeddingModelFamily.openai == "openai"
        assert KilnEmbeddingModelFamily.gemini == "gemini"

    def test_enum_membership(self):
        """Test that enum members are properly accessible"""
        assert "openai" in KilnEmbeddingModelFamily
        assert "gemini" in KilnEmbeddingModelFamily
        assert "invalid" not in KilnEmbeddingModelFamily
...
class TestEmbeddingModelName:
    ...
    def test_enum_membership(self):
        """Test that enum members are properly accessible"""
        assert "openai_text_embedding_3_small" in EmbeddingModelName
        assert "openai_text_embedding_3_large" in EmbeddingModelName
        assert "gemini_text_embedding_004" in EmbeddingModelName
        assert "invalid_model" not in EmbeddingModelName
...
class TestEmbeddingModelsList:
    ...
    def test_all_models_have_required_fields(self):
        ...
        for model in embedding_models:
            ...
            assert isinstance(model.family, KilnEmbeddingModelFamily)
