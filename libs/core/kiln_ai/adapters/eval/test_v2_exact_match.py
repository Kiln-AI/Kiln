...
class TestExactMatchBasic:
    def test_pass_case_sensitive(self):
        ...
        scores, _skip, _detail = ExactMatchEval(cfg).evaluate(_inp())
...
class TestExactMatchReferenceKey:
    def test_pass_with_reference_key(self):
        ...
        scores, skip, _ = ExactMatchEval(cfg).evaluate(inp)
        assert scores == {"score_a": 1.0}
        assert skip is None
    ...
    def test_fail_with_reference_key(self):
        ...
        scores, _skip, _ = ExactMatchEval(cfg).evaluate(inp)
        assert scores == {"score_a": 0.0}

    def test_missing_reference_data(self):
        cfg = _make_config(ExactMatchProperties(reference_key="answer"))
        inp = _inp(reference_data=None)
        scores, skip, _detail = ExactMatchEval(cfg).evaluate(inp)
        assert scores == {}
...
class TestExactMatchExpression:
    ...
    def test_undefined_expression_skips(self):
        ...
        scores, skip, detail = ExactMatchEval(cfg).evaluate(_inp())
        assert scores == {}
...
class TestExactMatchNoScores:
    def test_no_parent_eval_returns_empty(self):
        ...
        scores, skip, _ = ExactMatchEval(cfg).evaluate(_inp())
        assert scores == {}
        assert skip is None

    def test_no_output_scores_returns_empty(self):
        parent = Mock()
        parent.output_scores = []
        cfg = _make_config(ExactMatchProperties(expected_value="Hello world"))
        cfg.parent_eval.return_value = parent
...
class TestExactMatchMultipleScores:
    def test_all_scores_get_same_value(self):
        parent = Mock()
        parent.output_scores = [
            EvalOutputScore(
                name="score_a", instruction="a", type=TaskOutputRatingType.pass_fail
        ...
        cfg = _make_config(ExactMatchProperties(expected_value="Hello world"))
        cfg.parent_eval.return_value = parent
        scores, _, _ = ExactMatchEval(cfg).evaluate(_inp())
