...
async def test_multi_turn_drive_shares_one_instance_across_turns(
    ...
    """The drive runs with the instance in context for its whole conversation: every
    turn's tool call lands in the same copy, the leaf trace records that instance, and
    the trace is keyed by the synthetic variant."""
    ...
    cfg = _config(
        ...
        CodeEvalProperties(
            code=(
                ...
                "def score(output, synthetic_instance):\n"
                "    p = os.path.join(synthetic_instance['path'], 'counter.txt')\n"
                "    notes = open(p).read().splitlines() if os.path.exists(p) else []\n"
                "    return {'accuracy': 1.0 if notes == ['first note', 'second note'] else 0.0}\n"
            ),
            timeout_seconds=30,
        ),
    )
    tool_id = build_code_tool_id(real_tool.id)
    ...
    async def fake_drive(*, seed_prompt, target_task, turns, **_):
        ...
        for note in ("first note", "second note")[:turns]:
            ...
            seen_instances.append(ctx.instance.instance_id)
            result = await tool.run(
                ToolCallContext(synthetic_instance=ctx.instance), note=note
            )
            assert not result.is_error, result.output
            chain.append(
                TaskRun(
    ...
    with _patch(
        "kiln_ai.adapters.eval.eval_runner.drive_case_for_eval", new=fake_drive
    ):
        await _drain(_runner([cfg], run_config, provider))
