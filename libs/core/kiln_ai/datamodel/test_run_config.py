...
class RunConfigWrapper(BaseModel):
    """Wrapper model to test RunConfigProperties as a field in a parent model."""
...
class TestMcpRunConfigProperties:
    def test_direct_construction(self):
        config = McpRunConfigProperties(
            type="mcp",
            tool_reference=McpToolReference(server_id="my_server", tool_name="my_tool"),
        )
        assert config.type == "mcp"
        assert config.tool_reference.server_id == "my_server"
        assert config.tool_reference.tool_name == "my_tool"
    ...
    def test_type_serialized_in_output(self):
        ...
        assert data["tool_reference"]["server_id"] == "my_server"
        ...
        with pytest.raises(ValidationError):
            McpRunConfigProperties(  # type: ignore
                tool_reference=McpToolReference(
                    server_id="my_server", tool_name="my_tool"
                ),
            )
...
class TestBackwardCompatibility:
    """Tests that legacy JSON without a 'type' field loads as kiln_agent."""
...
class TestTaskRunConfigDecode:
    """Tests that TaskRunConfig correctly decodes MCP run configs, including via file loading path."""
    ...
    def test_mcp_task_run_config_from_dict(self):
        ...
        assert config.run_config_properties.tool_reference.server_id == "my_server"

    def test_mcp_task_run_config_loading_from_file(self):
        """Simulates loading an MCP TaskRunConfig from a file.

        Ensures upgrade_old_entries doesn't inject structured_output_mode
        into MCP configs.
        """
        ...
        assert isinstance(config.run_config_properties, McpRunConfigProperties)
        assert config.run_config_properties.type == "mcp"
        assert config.run_config_properties.tool_reference.tool_name == "my_tool"
    ...
    def test_legacy_kiln_agent_task_run_config_loading_from_file(self):
        """Simulates loading a legacy TaskRunConfig (no type field) from a file.

        Ensures upgrade_old_entries injects both type and structured_output_mode.
        """
        from kiln_ai.datamodel.task import TaskRunConfig
...
class TestSaveLoadBothTypes:
    """Tests serialization/deserialization roundtrips for both types."""
    ...
    def test_mcp_roundtrip(self):
        original = McpRunConfigProperties(
            type="mcp",
            tool_reference=McpToolReference(
                server_id="prod_server", tool_name="search_tool"
            ),
        )
        dumped = json.loads(json.dumps(original.model_dump()))
        restored = run_config_adapter.validate_python(dumped)
        ...
        assert restored.tool_reference.server_id == "prod_server"
        assert restored.tool_reference.tool_name == "search_tool"
