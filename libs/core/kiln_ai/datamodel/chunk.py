...
def validate_semantic_chunker_properties(
    ...
    # Required properties
    model_provider = properties.get("model_provider")
    if model_provider is None:
        raise ValueError("model_provider is required for semantic chunker.")

    model_provider_name = properties.get("model_provider_name")
    ...
    if not isinstance(model_provider_name, str):
        raise ValueError("model_provider_name must be a string.")
...
class ChunkerConfig(KilnParentedModel):
    ...
    def model_provider(self) -> str | None:
        if self.properties.get("model_provider") is None:
            return None
        if not isinstance(self.properties["model_provider"], str):
            raise ValueError("Model provider must be a string.")
        return self.properties["model_provider"]

    def model_provider_name(self) -> str | None:
        if self.properties.get("model_provider_name") is None:
            return None
        if not isinstance(self.properties["model_provider_name"], str):
            raise ValueError("Model provider name must be a string.")
        return self.properties["model_provider_name"]
    ...
    def breakpoint_percentile_threshold(self) -> float | None:
        if self.properties.get("breakpoint_percentile_threshold") is None:
            return None
        if not isinstance(
            self.properties["breakpoint_percentile_threshold"], (int, float)
        ):
            raise ValueError("Breakpoint percentile threshold must be a number.")
        return float(self.properties["breakpoint_percentile_threshold"])
