...
def test_continue_task_run_success(client, task_run_setup):
    """Test continuing a task run from a prior trace."""
    task = task_run_setup["task"]
    ...
    with (
        ...
        response = client.post(
            f"/api/projects/{project.id}/tasks/{task.id}/runs/{prior_run.id}/continue",
            json={
                "run_config_properties": run_config,
                "plaintext_input": "Follow up question",
            },
        )
    ...
    res = response.json()
    assert res["id"] is not None
    assert res["input"] == "Follow up question"

    # Verify adapter.invoke was called with prior_trace
    mock_invoke.assert_called_once()
    call_args = mock_invoke.call_args
    assert call_args[0][0] == "Follow up question"
    assert "prior_trace" in call_args[1]
    assert len(call_args[1]["prior_trace"]) == 2


def test_continue_task_run_no_trace(client, task_run_setup):
    """Test continuing a task run fails when prior run has no trace."""
    task = task_run_setup["task"]
    prior_run = task_run_setup["task_run"]
    project = task_run_setup["project"]
    ...
    assert response.status_code == 400
    assert "no trace available" in response.json()["message"].lower()


def test_continue_task_run_not_found(client, task_run_setup):
    """Test continuing a task run fails when prior run doesn't exist."""
    task = task_run_setup["task"]
    project = task_run_setup["project"]
    ...
    with patch("kiln_server.run_api.task_and_run_from_id") as mock_task_and_run:
        ...
        response = client.post(
            f"/api/projects/{project.id}/tasks/{task.id}/runs/nonexistent_run_id/continue",
            json={
                "run_config_properties": run_config,
                "plaintext_input": "Follow up question",
            },
...
def test_continue_task_run_no_input(client, task_run_setup):
    """Test continuing a task run fails when no input is provided."""
    task = task_run_setup["task"]
    prior_run = task_run_setup["task_run"]
    project = task_run_setup["project"]
