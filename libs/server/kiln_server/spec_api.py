...
def resolve_available_spec_name(
    ...
    looping forever).
    """
    taken = {generate_spec_eval_tags(name).eval_tag for name in existing_names}
    if generate_spec_eval_tags(candidate).eval_tag not in taken:
    ...
    for i in range(2, 100):
        ...
        if generate_spec_eval_tags(variant).eval_tag not in taken:
