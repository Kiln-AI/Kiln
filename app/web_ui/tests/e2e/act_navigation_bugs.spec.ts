...
- The spec detail page title is `Auto-Eval: <name>` (rendered as an h-level heading
  via AppPage). Use getByRole("heading", { name: `Auto-Eval: <name>` }) to wait.
- Same-route client-side nav must go through an <a> click — page.goto would
  remount the component and mask the bug.

## Assertions
- Visiting specA first shows "Auto-Eval: specA.name".
- After client-side nav to specB's URL, the URL updates and the heading becomes
  "Auto-Eval: specB.name" (and the old specA heading is gone).
*/
test.fixme(
  ...
  async ({ page, apiRequest, registeredUser, seededProjectWithTask }) => {
    ...
    await expect(
      page.getByRole("heading", { name: `Auto-Eval: ${specA.name}` }),
    ).toBeVisible()
    ...
    await expect(
      page.getByRole("heading", { name: `Auto-Eval: ${specB.name}` }),
    ).toBeVisible()
    await expect(
      page.getByRole("heading", { name: `Auto-Eval: ${specA.name}` }),
    ).toHaveCount(0)
