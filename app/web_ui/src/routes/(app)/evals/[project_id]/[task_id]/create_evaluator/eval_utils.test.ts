...
describe("generate_issue_eval_tag", () => {
  beforeEach(() => {
  ...
  it("converts normal names to lowercase with underscores", () => {
    const result = generate_issue_eval_tag("Test Name")
    expect(result).toBe("test_name")
  })

  it("handles single words without spaces", () => {
    const result = generate_issue_eval_tag("TestName")
    expect(result).toBe("testname")
  })

  it("handles multiple spaces", () => {
    const result = generate_issue_eval_tag("Test   Multiple   Spaces")
    expect(result).toBe("test___multiple___spaces")
  })

  it("handles special characters (keeps them as is)", () => {
    const result = generate_issue_eval_tag("Test-Name_With.Special@Chars")
    expect(result).toBe("test-name_with.special@chars")
  })

  it("handles numbers in the name", () => {
    const result = generate_issue_eval_tag("Test 123 Name")
  ...
  it("truncates names longer than 32 characters", () => {
    ...
    const result = generate_issue_eval_tag(longName)
  ...
  it("handles exactly 32 character names", () => {
    ...
    const result = generate_issue_eval_tag(exactName)
  ...
  it("handles names shorter than 32 characters", () => {
    ...
    const result = generate_issue_eval_tag(shortName)
  ...
  it("generates random tag when name is empty string", () => {
    ...
    const result = generate_issue_eval_tag("")
  ...
  it("preserves underscores in original names", () => {
    const result = generate_issue_eval_tag("Test_Name_With_Underscores")
    expect(result).toBe("test_name_with_underscores")
  })

  it("handles Unicode characters", () => {
    const result = generate_issue_eval_tag("Test Café Naïve")
    expect(result).toBe("test_café_naïve")
  })

  it("handles mixed case with numbers and spaces", () => {
    const result = generate_issue_eval_tag("API Test 2024 Version")
  ...
  it("generates different random values on multiple calls with empty string", () => {
    ...
    const result1 = generate_issue_eval_tag("")
    const result2 = generate_issue_eval_tag("")
  ...
  it("only generates random tags for truly empty strings", () => {
    ...
    expect(generate_issue_eval_tag(" ")).toBe("_")
    expect(generate_issue_eval_tag("_")).toBe("_")
    expect(generate_issue_eval_tag("a")).toBe("a")

    // Only this should generate a random tag
    expect(generate_issue_eval_tag("")).toBe("issue_55000")
  })
})
