<script lang="ts">
  ...
  function toggle_selection(run_id: string): boolean {
    const was_selected = selected_documents.has(run_id)
    if (was_selected) {
      selected_documents.delete(run_id)
    } else {
      selected_documents.add(run_id)
    }
    // Reactivity trigger
  ...
  function open_document(document_id: string) {
    const url = `/documents/${project_id}/${document_id}`
    goto(url)
  }
  ...
  function select_range(run_id: string, event: MouseEvent) {
    ...
    const run_index = documents?.findIndex((document) => document.id === run_id)
    if (
  ...
  function select_all_clicked(event: Event) {
    ...
    if (select_document === "all" || select_document === "some") {
      ...
      filtered_documents?.forEach((run) => {
        if (run.id) {
          selected_documents.add(run.id)
        }
      })
    }
    selected_documents = selected_documents
          ...
          <tbody>
            {#each (filtered_documents || []).slice((page_number - 1) * page_size, page_number * page_size) as document}
              ...
              >
                ...
                <td>{format_size(document.original_file.size)}</td>
