<script lang="ts">
  ...
  // Improvement direction per score key, so the radar can leave out the scores that
  // don't share its "further out is better" grammar
  $: scoreDirections = (() => {
    const directions: Record<string, string> = {}
    for (const [eval_id, evalData] of Object.entries(eval_data_cache)) {
      for (const score of evalData?.output_scores || []) {
        directions[`${eval_id}::${string_to_json_key(score.name)}`] =
          score.direction
      }
    }
    return directions
