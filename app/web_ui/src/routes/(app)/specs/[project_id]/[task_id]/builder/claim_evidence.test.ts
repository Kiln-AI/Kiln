...
describe("single_turn_sections — the two sections a single-turn review shows", () => {
  const tool_call = {
    id: "call_1",
    type: "function",
    function: { name: "lookup_policy", arguments: '{"topic": "returns"}' },
  }

  it("drops the system prompt and the input turn from the output", () => {
    // The input has its own field, so its turn must not appear again as a
    // row, and the system prompt is task config rather than graded content.
    const t = trace({
      trace: [
        { role: "system", content: "You are a support agent." },
        { role: "user", content: "What's the return window?" },
        ...
        {
          role: "tool",
          content: '{"output": "30 days"}',
          tool_call_id: "call_1",
        },
        { role: "assistant", content: "Our return window is 30 days." },
      ] as unknown as TraceMessage[],
    })
    const sections = single_turn_sections(t)
    expect(sections.input).toBe(t.raw_input)
    expect((sections.rows ?? []).map((m) => m.role)).toEqual([
      "assistant",
      "tool",
      "assistant",
    ])
  ...
  it("drops a system turn that sits AFTER the input", () => {
    // Slicing at the first user turn already hides the usual leading system
    // prompt; a provider that injects one mid-trace must not reintroduce it.
    const t = trace({
      trace: [
        { role: "user", content: "What's the return window?" },
        { role: "assistant", content: "Let me check." },
        { role: "system", content: "Reminder: cite the policy." },
        { role: "assistant", content: "Our return window is 30 days." },
      ] as TraceMessage[],
    })
    const sections = single_turn_sections(t)
    ...
    // The expansion index counts the rows on screen, not the raw trace.
    expect(sections.expand_indices).toEqual([1])
  })

  it("opens the FINAL assistant row only", () => {
    const t = trace({
      trace: [
        { role: "user", content: "What's the return window?" },
    ...
    // Row 3 of [assistant, assistant, tool, assistant] — the answer being
    // graded, not the first assistant message and not every assistant message.
    expect(single_turn_sections(t).expand_indices).toEqual([3])
  })

  it("renders a single-message output as a plain field, with no rows", () => {
    // An echoed pair, a stored record pair and a traceless run all leave one
    // message after the input: there is no sequence for an accordion to order.
    const echo = trace({
      trace: [
        { role: "system", content: "You are a support agent." },
        { role: "user", content: "What's the return window?" },
    ...
    const echo_sections = single_turn_sections(echo)
    expect(echo_sections.rows).toBeNull()
    expect(echo_sections.expand_indices).toEqual([])
    expect(echo_sections.output).toBe("Our return window is 30 days.")

    const traceless = single_turn_sections(trace({ trace: null }))
    expect(traceless.rows).toBeNull()
    expect(traceless.output).toBe("Our return window is 30 days.")
  })

  it("falls back to raw_output when the final assistant carries no content", () => {
    // A function-calling run puts the answer in the tool call's arguments and
    // stores content: null. Reading the message alone shows the reviewer an
    // empty output for a run that answered.
    const t = trace({
      raw_output: '{"window_days": 30}',
      trace: [
        { role: "user", content: "What's the return window?" },
        {
          role: "assistant",
    ...
    expect(single_turn_sections(t).output).toBe('{"window_days": 30}')
  })

  it("prefers the final assistant's own content over raw_output", () => {
    // raw_output can be a lossy flattening; the message is the exact answer.
    const t = trace({
    ...
    expect(single_turn_sections(t).output).toBe("hi")
  })

  it("treats a whitespace-only answer as no answer", () => {
    const t = trace({
      raw_output: "the real answer",
      trace: [
        { role: "user", content: "hello" },
        { role: "assistant", content: "   " },
      ] as TraceMessage[],
    })
    expect(single_turn_sections(t).output).toBe("the real answer")
  })

  it("reports a missing output on the section, keeping the input", () => {
    // The Output has nothing, but the Input does. Throwing here would take the
    // input off screen with it, so the failure is scoped to its own section.
    const sections = single_turn_sections(
      trace({
        raw_output: "",
        trace: [
          { role: "user", content: "What's the return window?" },
    ...
    expect(sections.output_error).toBe("This trace recorded no output.")
    expect(sections.input).toBe("What's the return window?")
    expect(sections.rows).toBeNull()
  })

  it("still throws when the trace has nothing at all", () => {
    // No transcript and no raws: there is no input to keep either.
    expect(() =>
      single_turn_sections(
        trace({ trace: null, raw_input: "", raw_output: "" }),
      ),
  ...
  it("shows a lone tool call rather than reporting nothing", () => {
    // A run whose answer never resolved still made a call, and the rows can
    // render it. Reporting "no output" over renderable content would be the
    // fail-loud rule firing on something that is not missing.
    const sections = single_turn_sections(
      trace({
        raw_output: "",
        trace: [
    ...
    expect(sections.output_error).toBeNull()
    expect(sections.rows).toHaveLength(1)
    expect(sections.expand_indices).toEqual([0])
  })

  it("keeps the raw_output fallback ahead of the tool-call rows", () => {
    // The ruled order: a function-calling run whose answer IS raw_output reads
    // as a field showing that answer, not as an accordion holding the call.
    const sections = single_turn_sections(
      trace({
        raw_output: '{"window_days": 30}',
        trace: [
    ...
    expect(sections.rows).toBeNull()
    expect(sections.output).toBe('{"window_days": 30}')
  })

  it("keeps a user turn that comes after the first one", () => {
    // The section shows what the trace holds after the INPUT turn. A later
    // user turn is something the run really did; dropping it would hide it.
    const sections = single_turn_sections(
      trace({
        trace: [
          { role: "user", content: "What's the return window?" },
          { role: "assistant", content: "Thirty days." },
          { role: "user", content: "And for exchanges?" },
    ...
    expect(sections.expand_indices).toEqual([2])
  })

  it("resolves the field's answer text from the one row it renders", () => {
    // Only the single-message output reaches a field, so that is the shape
    // this resolves: the row's own content, else raw_output, else nothing.
    const row = (content: string | null): TraceMessage =>
      ({ role: "assistant", content }) as unknown as TraceMessage
    expect(
      single_turn_output_text(trace({ raw_output: "raw" }), row("the answer")),
    ).toBe("the answer")
    expect(
      single_turn_output_text(trace({ raw_output: "raw" }), row(null)),
    ).toBe("raw")
    expect(single_turn_output_text(trace({ raw_output: "" }), row(null))).toBe(
      null,
    )
    // A row that is not the model answering carries no answer text.
    expect(
      single_turn_output_text(trace({ raw_output: "raw" }), {
        role: "tool",
        content: '{"output": "30 days"}',
        tool_call_id: "call_1",
      } as unknown as TraceMessage),
  ...
  it("keeps the rows for a lone message that also recorded reasoning", () => {
    // A field renders the answer text only, so dropping to one would delete
    // the model's thinking from the surface without saying so.
    const t = trace({
      trace: [
        { role: "user", content: "What's the return window?" },
        {
          ...
          reasoning_content: "The policy page says 30.",
        },
      ] as unknown as TraceMessage[],
    })
    const sections = single_turn_sections(t)
  ...
  it("does not demand an answer text when the rows are on screen", () => {
    // A loop whose final message is a tool call rather than a sentence has a
    // rendered output; failing it as blank would strand a real trace.
    const t = trace({
      raw_output: "",
      trace: [
        { role: "user", content: "What's the return window?" },
    ...
    expect(single_turn_sections(t).rows).toHaveLength(2)
  })
})

describe("single_turn_sections_resolver — stable across parent churn", () => {
  const messages = [
    { role: "user", content: "What's the return window?" },
    { role: "assistant", content: "Thirty days." },
    { role: "assistant", content: "Confirmed: thirty days." },
  ] as TraceMessage[]

  it("returns the SAME object when only unrelated fields changed", () => {
    // The review page rebuilds its whole trace list whenever a background
    // claims build lands. A fresh sections object there would hand the rows a
    // new array and remount them, closing what the reviewer had open.
    const resolve = single_turn_sections_resolver()
    const before = trace({ trace: messages, claims_state: "unbuilt" })
    const first = resolve(before)
    const patched = {
      ...before,
      claims_state: "built" as const,
      claims: [claim()],
    }
    const second = resolve(patched)

    expect(second).toBe(first)
    expect(second.sections).toBe(first.sections)
    expect(second.sections?.rows).toBe(first.sections?.rows)
  })

  it("recomputes when the content the sections render changes", () => {
    const resolve = single_turn_sections_resolver()
    const first = resolve(trace({ trace: messages }))
    // A re-judge round gives the case a new id and fresh raws.
    const rejudged = resolve(
      trace({
        trace_id: "round2_case_0",
        trace: messages,
        raw_output: "Thirty days, confirmed.",
      }),
    )
    expect(rejudged).not.toBe(first)
    expect(rejudged.sections?.trace_id).toBe("round2_case_0")
  })

  it("recomputes when the stored transcript itself is replaced", () => {
    const resolve = single_turn_sections_resolver()
    const first = resolve(trace({ trace: messages }))
    const replaced = resolve(trace({ trace: [...messages] }))
    expect(replaced).not.toBe(first)
  })

  it("reports an unrenderable trace instead of throwing", () => {
    const resolve = single_turn_sections_resolver()
    const result = resolve(
      trace({ trace: null, raw_input: "", raw_output: "" }),
    )
    expect(result.sections).toBeNull()
    expect(result.error).toContain("no transcript")
  })
})
