...
<div class="max-w-[900px]">
  <AppPage
    ...
    breadcrumbs={[
      ...
      {
        label: "Tool Server",
        href: `/tools/${project_id}/tool_servers/${tool_id?.split("::")[2] ?? null}`,
      },
    ]}
  >
    {#if tool_missing}
            ...
            {#if incompatible_count > 0}
              <Warning
                warning_message={`${
                  ...
                  tool_id
                    ? `\n[Create a new task using this tool's schema](/tools/${project_id}/create_task_from_tool?tool_id=${encodeURIComponent(tool_id)})`
                    : ""
                }`}
