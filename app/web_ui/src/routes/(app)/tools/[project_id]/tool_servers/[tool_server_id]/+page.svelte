<script lang="ts">
  ...
  import { load_available_tools, ui_state } from "$lib/stores"
  ...
  function handleCreateTask(tool_name: string) {
    ...
    goto(
      `/tools/${project_id}/create_task_from_tool?tool_id=${encodeURIComponent(
  ...
  $: direct_mcp_href = selected_tool_name
    ? `/tools/${project_id}/add_tool_to_task?tool_id=${encodeURIComponent(
    ...
    secretKeys.forEach((key: string) => {
      ...
      if (missingSecrets.includes(key)) {
        properties.push({
          ...
          value: "Value missing",
          error: true,
          link: `/tools/${project_id}/edit_tool_server/${tool_server_id}`,
        })
  ...
  async function update_archive(is_archived: boolean) {
    try {
      ...
      switch (current_tool_server.type) {
        ...
        case "kiln_task": {
          // Kiln task tools is handled by /tools/[project_id]/kiln_task/[tool_server_id] page
          break
        }
        default: {
...
<div class="max-w-[1400px]">
  <AppPage
    ...
    action_buttons={[
      {
        label: "Edit",
        href: `/tools/${project_id}/edit_tool_server/${tool_server_id}`,
      },
      {
