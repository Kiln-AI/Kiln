<script lang="ts">
  ...
  function button_label(github_starred: boolean) {
    if (!github_starred) {
...
<div class="md:w-[400px] max-w-[400px] mx-auto flex flex-col gap-6 mt-12">
  <button
  ...
  >
    ...
    <div class="pl-1 grow">
      <div class="text-left text-lg font-medium">Star us on GitHub</div>
      <div class="text-sm font-light text-left">
        Helps others discover and support Kiln
      </div>
    </div>
    {#if github_starred}
...
>
  <a href="/setup/connect_providers">
    <button
      class="btn {github_starred ? 'btn-primary' : ''} w-full min-w-[130px]"
    >
      {button_label(github_starred)}
    </button></a
  >
</div>
