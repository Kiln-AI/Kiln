...
// Typed as a Record over the union, so adding a rating type is a type error
// here until it gets a label. That keeps the component's dropped {:else}
// fallback safe: a type without a branch would render nothing.
const labels: Record<TaskOutputRatingType, string> = {
  five_star: "1 to 5",
  pass_fail: "pass/fail",
...
describe("OutputTypeTablePreview", () => {
  const cases = Object.entries(labels) as [TaskOutputRatingType, string][]

  it.each(cases)("labels %s as %s", (output_score_type, label) => {
