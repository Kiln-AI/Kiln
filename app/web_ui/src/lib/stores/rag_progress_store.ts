...
// we need to call this in every component that uses the rag progress store
// because the project does not set the project state automatically
export function ensure_rag_store_initialized(project_id: string) {
  if (get(ragProgressStore)[project_id]) {
    return
  }
