...
class CreateSpecWithCopilotRequest(BaseModel):
    ...
    run_config_id: str | None = Field(
        default=None,
        description="The task run config this spec is written against. Its "
        "tools and skills describe the target task when the legacy path "
        "generates examples. Omit to use the task's default run config.",
    )
