...
# ---- get_failing_train_examples endpoint ----


@pytest.mark.asyncio
async def test_get_failing_train_examples_success(
    client, mock_task, mock_task_from_id, mock_eval, mock_eval_config
):
    from kiln_ai.adapters.eval.failing_examples import FailingExample, FindFailingResult

    fake_result = FindFailingResult(
        examples=[
            FailingExample(
                dataset_id="d1",
                scores={"score1": 1.0, "overall_rating": 2.0},
                feedback="output was off topic",
            ),
        ],
        num_judged=4,
        train_set_size=12,
    ...
    with patch(
        "app.desktop.studio_server.eval_api.find_failing_train_examples",
        new=AsyncMock(return_value=fake_result),
    ) as mock_find:
        response = client.post(
            "/api/projects/project1/tasks/task1/evals/eval1/eval_config/eval_config1/failing_train_examples",
            json={"count": 1, "max_samples": 10, "threshold": 0.8},
        )

    assert response.status_code == 200
    body = response.json()
    assert body["num_judged"] == 4
    assert body["train_set_size"] == 12
    assert body["hit_cap"] is False
    assert len(body["examples"]) == 1
    assert body["examples"][0]["dataset_id"] == "d1"
    assert body["examples"][0]["scores"] == {"score1": 1.0, "overall_rating": 2.0}
    assert body["examples"][0]["feedback"] == "output was off topic"

    # Called with the resolved eval config and the request params
    mock_find.assert_awaited_once()
    call = mock_find.await_args
    assert call.args[0].id == "eval_config1"
    assert call.kwargs["count"] == 1
    assert call.kwargs["max_samples"] == 10
    assert call.kwargs["threshold"] == 0.8


@pytest.mark.asyncio
async def test_get_failing_train_examples_defaults(
    client, mock_task, mock_task_from_id, mock_eval, mock_eval_config
):
    from kiln_ai.adapters.eval.failing_examples import FindFailingResult

    fake_result = FindFailingResult(
        examples=[], num_judged=0, train_set_size=0, hit_cap=False
    )
    ...
    assert response.status_code == 200
    call = mock_find.await_args
    assert call.kwargs["count"] == 5
    assert call.kwargs["max_samples"] == 50
    assert call.kwargs["threshold"] == 0.75


def test_get_failing_train_examples_no_train_set(client, mock_task, mock_task_from_id):
    # Patch eval_from_id to return an eval without a train set filter
    no_train_eval = Eval(
        id="eval1",
        name="No Train Eval",
    ...
    with patch(
        "app.desktop.studio_server.eval_api.eval_from_id",
        return_value=no_train_eval,
    ):
        response = client.post(
            "/api/projects/project1/tasks/task1/evals/eval1/eval_config/eval_config1/failing_train_examples",
            json={"count": 1, "max_samples": 5},
        )

    assert response.status_code == 400
    assert "train set" in response.json()["message"].lower()


def test_get_failing_train_examples_eval_not_found(
    client, mock_task, mock_task_from_id
):
    response = client.post(
        "/api/projects/project1/tasks/task1/evals/missing/eval_config/eval_config1/failing_train_examples",
        json={"count": 1, "max_samples": 5},
    )
    assert response.status_code == 404


def test_get_failing_train_examples_config_not_found(
    client, mock_task, mock_task_from_id, mock_eval
):
    response = client.post(
        "/api/projects/project1/tasks/task1/evals/eval1/eval_config/missing/failing_train_examples",
        json={"count": 1, "max_samples": 5},
    )
    assert response.status_code == 404


def test_get_failing_train_examples_invalid_counts(
    client, mock_task, mock_task_from_id, mock_eval, mock_eval_config
):
    response = client.post(
        "/api/projects/project1/tasks/task1/evals/eval1/eval_config/eval_config1/failing_train_examples",
        json={"count": 5, "max_samples": 2},
    )
    assert response.status_code == 422
