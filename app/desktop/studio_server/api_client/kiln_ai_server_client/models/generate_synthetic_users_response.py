...
class GenerateSyntheticUsersResponse:
    ...
    One case per scenario that produced a usable user. A case with a blank
    `synthetic_user_info` or `seed_prompt` is left out, so `cases` can be
    shorter than `case_scenarios`.

        Attributes:
            cases (list[SyntheticUserCase]): Generated cases, in scenario order.
    """
