...
- [ ] **Phase 1 — Submodel & helper (pure core).** `KilnArtifactProvenance` in `datamodel/provenance.py` (context-aware, lenient-on-load validators) + `validate_derived_from_ids` helper; export from `datamodel/__init__.py`; unit tests (`test_provenance.py`) covering the create-vs-load validator matrix and the helper.
