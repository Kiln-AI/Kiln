...
| 27-R22 | helper | MET | — | `kiln.get_tool_results(trace) -> list[dict]` with `tool_call_id` and `content` | §3.1 | `eval_helpers.py:35-44` — returns entries where `role == "tool_result"` or `type == "tool_result"` | — |
