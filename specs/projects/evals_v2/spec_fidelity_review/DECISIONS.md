...
| D16 | Code score range-validated at persist, not execution → test pane accepts out-of-range. **Moved to `evals_v2_ui_polish` (Phase 10)** — its only user-visible surface is that project's create-flow test pane. | 27-R14 | minor | 3 → ui_polish | Project: evals_v2_ui_polish | Ship-blocker | **ratified** |
...
- **▶ Project 3 — "Evals V2 cleanup"** — Plan → `specs/projects/evals_v2_cleanup/project_overview.md`. Members: **D14, D15, D27, D28, D29, D30, D31, D35**. _(D16 moved to `evals_v2_ui_polish` Phase 10.)_
  ...
  - D16 (Ship-blocker) — **moved to `specs/projects/evals_v2_ui_polish/` (Phase 10)**, since its only user-visible surface is that project's create-flow test pane. Plan unchanged: extract `EvalRun.validate_scores` range logic (`eval.py:531-587`) into a shared `validate_scores_against_output_scores(...)`; call from `EvalRun` + `test_v2_eval`; ~10 net LOC, zero per-type maintenance.
