...
| D16 | Code score range-validated at persist, not execution → test pane accepts out-of-range. **Moved out of cleanup to `evals_v2_ui_polish` (Phase 10)** — its only user-visible surface is that project's create-flow test pane. Implemented on `scosman/evals_v2_ui_fix`. | 27-R14 | minor | 3 → ui_polish | Project: evals_v2_ui_polish | Ship-blocker | **ratified** |
...
- **▶ Project 3 — "Evals V2 cleanup"** — Plan → `specs/projects/evals_v2_cleanup/project_overview.md`. Members: **D14, D15, D27, D28, D29, D30, D31, D35**. _(D16 moved to `evals_v2_ui_polish` Phase 10; implemented on `scosman/evals_v2_ui_fix`.)_ **Fully parallel-safe with Project 1** (no shared files; D27–D30 share `eval.py` with Project 1's D21 but different classes — coordinate, low risk).
