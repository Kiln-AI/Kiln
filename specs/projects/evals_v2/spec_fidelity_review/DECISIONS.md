...
| D16 | Code score range-validated at persist, not execution → test pane accepts out-of-range | 27-R14 | minor | 3 | Cleanup | Ship-blocker | **ratified** |
...
| D32 | Build a **read-only config-detail view** for V2 configs: shows a saved config's type + key properties so a user can see what a candidate does before cloning. Today non-`llm_judge` V2 configs show "No description provided." (§4.3) | 70b-R14 | minor | 5 | Cleanup | Ship-blocker | **ratified** |
...
- **▶ Project 3 — "Evals V2 cleanup"** — Plan → `specs/projects/evals_v2_cleanup/project_overview.md`. Members: **D14, D15, D16, D27, D28, D29, D30, D31, D32, D35**.
  ...
  - D16 (Ship-blocker) — validate returned scores' range in the test pane. Extract `EvalRun.validate_scores` range logic (`eval.py:530-587`) into a shared `validate_scores_against_output_scores(...)`; call from `EvalRun` + `test_v2_eval` (`eval_api.py:996-1003`); ~10 net LOC, zero per-type maintenance.
  ...
  - D32 (Ship-blocker) — read-only config-detail view for V2 configs (largest item; net-new small view). *View surface — no conflict with Projects 1/2.*
