...
- **EvalRun additive fields.** `eval_input_id`, `skipped_reason` — all optional, V1 EvalRuns load them as `None`. No reference snapshot: A2.7's reference data is assembled per item into the runtime `EvalTaskInput`. Skip-with-reason relaxes the score/output validators when set. Detail: `components/10 §5`, `components/45`.
