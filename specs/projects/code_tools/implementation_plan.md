...
- **(dependency) Trust integration** — owned by the parallel project-trust work; NOT built in this project. Code Tools must not merge to main until that trust dialog lands on main and is wired into the stopgap call sites (`PythonCodeTool.run`, the transient test endpoint) and the UI trust dialog, replacing the stopgaps. Security-related strings also require human sign-off before their `# TODO`s are removed. Enforced by the merge-blocking `TODO(trust)` comments in the codebase.
