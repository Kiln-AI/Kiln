...
- [x] **Phase 3 — Task creation/edit form.** Add `turn_mode` segmented control to `edit_task.svelte`, hide input/output schema sections when multiturn, render read-only on the edit page. Regenerate the OpenAPI client types. Frontend tests for the toggle behavior and conditional rendering.

- [x] **Phase 4 — Multiturn run flow + conversation view.** Branch `/run/+page.svelte` on send-success to navigate to the run-detail page for multiturn tasks. Build `ConversationView`, `TurnCard`, `Composer` under `lib/ui/conversation/`. Branch the dataset run-detail page to render the conversation + composer for multiturn tasks. Each Send creates a new TaskRun and replaces the URL. Frontend tests for the three new components and the page-level branch.
