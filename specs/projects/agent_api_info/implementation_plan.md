...
- [ ] Phase 5: `agent_overview` endpoint in new `agent_api.py` — all response models, block helpers (`_truncate_to_words`, `_split_tool_and_skill_ids`, `_dataset_stats`, `_docs_stats`, per-section blocks, `_connected_providers_block`), route wiring via `connect_agent_api(app)`, and tests (helpers, per-block aggregators, happy path, empty task, truncation edges).
