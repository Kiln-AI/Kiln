...
**Auto-generated `Skill.description`:** The `Skill.description` field (used for tool discovery, max 1024 chars) is editable on the DocSkill form as the second field after skill name, labeled "A description of when an agent should use this skill." with a tooltip: "This is shown to the agent to help it decide when to load the skill. Keep it concise but informative." If not provided by the user, it is auto-generated: `"A skill providing access to a set of documents."` — or if tags are set: `"A skill providing access to a set of documents tagged '{tag1}', '{tag2}'"` (comma-separated, each in single quotes).
...
- **Skill description** (textarea, optional — "A description of when an agent should use this skill." with tooltip)
- **Document tags** (tag selector component, reused from RAG)
- **Skill body intro** (textarea, pre-filled with template default — longer description inserted into SKILL.md)
- **Extractor** (pre-filled from template, editable)
- **Chunker** (pre-filled from template, editable)
- **Advanced Options** collapsible section:
  - "Custom Document Skill Name" (display name, not used by agent)
