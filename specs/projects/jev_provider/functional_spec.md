...
Run config fields `temperature`, `top_p`, `thinking_level` are ignored silently. Chain-of-thought prompt generators are accepted: Jev has no thinking step, so it produces no `chain_of_thought` intermediate output, but the generator's thinking instructions are appended to the system prompt under a `# Thinking Instructions` heading rather than dropped. They are content, not just a procedure — a legacy LLM-as-Judge config carries its eval steps there and nowhere else, so dropping them would make every judge config on an eval the same judge. The composition is exactly `build_prompt_for_ui`'s, so Kiln's prompt viewer shows the text Jev actually receives. Prompt content (simple, few-shot, multi-shot, saved prompts, fine-tune prompts) is used in full.
...
- **Legacy LLM-as-Judge** (`g_eval.py` with `config_type == llm_as_judge`): the judge task's output schema is built by `build_score_schema(allow_float_scores=False)`, which produces only shapes in the mapping table. The adapter is selected by provider, so the eval code does not change. The `SIMPLE_CHAIN_OF_THOUGHT` prompt id it uses is accepted; Jev runs no thinking step, but the eval steps that prompt id carries as thinking instructions are appended to the system prompt, because `eval_steps` is the config's only required property. Scores come from the discrete dict output as today.
