...
- [ ] Phase 1: Provider plumbing. `ModelProviderName.typesafe`, `typesafe_api_key` config, `provider_tools` (friendly name, `provider_warnings`, LiteLLM cases that raise), `utils/litellm.py` case, `connect_typesafe` (verify first whether `GET /v1/models` rejects a bad key; else the minimal-POST check) plus connect/disconnect dispatch in `provider_api.py`, and the standard provider tests.
