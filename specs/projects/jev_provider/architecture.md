...
5. `system_prompt = self.prompt_builder.build_prompt(include_json_instructions=False)`. No `skills` argument: a run config carrying a skill is rejected in step 2. Never calls `chain_of_thought_prompt()`.
