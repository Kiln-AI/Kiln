...
5. `system_prompt = self.prompt_builder.build_prompt(include_json_instructions=False)`, then `chain_of_thought_prompt()` appended under `# Thinking Instructions` when it returns anything, matching `build_prompt_for_ui`'s composition. Jev has no thinking step, but a legacy LLM-as-Judge config's `eval_steps` reach the model only as thinking instructions, so dropping them would make every judge config on an eval identical. No `skills` argument: a run config carrying a skill is rejected in step 2.
