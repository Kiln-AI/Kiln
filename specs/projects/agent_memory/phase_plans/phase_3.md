...
3. **Agent policy** (decision 11): all six agent-allowed, **no approval gate** — including `delete` (an agent that can't prune its own landfill defeats the purpose). Note: the codebase `ALLOW_AGENT` is a REST/openapi_extra annotation and this project has **no REST surface** (decision 12); realize "agent-allowed, no approval" in whatever mechanism the harness uses for tool availability/approval, not via the REST policy constant.
