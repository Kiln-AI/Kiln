...
- **Writes are atomic.** Unlike the rest of Kiln (whose write safety comes from a single-process, API-level git-sync write lock), the memory store is lock-free and multi-process, so a reader must never observe a half-written file. `Memory` overrides `save_to_file` to write to a temp file and `os.replace` it into place; every reader sees a complete file (old or new), never a torn one. (See architecture §4.)
- **Updates to the *same* record are last-writer-wins.** This is accepted: the dangerous race was parallel appends, not parallel edits of one record. Atomic replace guarantees the winner/loser boundary is a whole file, never a merge. Guidance (skills) steers agents toward "add a `stale` tag + write a correction" over rewriting others' records.
- **Reads (list/get/summary) are lock-free** directory scans; because writes are atomic, they tolerate concurrent writers (a record appearing/disappearing mid-scan is fine — the scan sees a consistent-enough snapshot for orientation, and never reads a partial file).
