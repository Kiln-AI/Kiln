...
> **This phase's code is not on `claude/eval-splits-evals-v2`.** Everything below is accurate as
> built and reviewed — the phase completed on `claude/eval-splits-v1-v2-q38412` as commit
> `369a32ef8` — but both files it changes, `app/desktop/studio_server/jobs/workers/eval.py` and
> `jobs/api.py`'s eval route, belong to the still-draft PR #1517 and do not exist on
> `scosman/evals_v2`. This plan is kept as the record and as the instruction sheet for re-landing
> the phase on a tree that has the worker. `phase5_mutation_sweep.py` is not kept: every one of its
> mutations edits one of those two files. See `implementation_plan.md`'s phase 5 entry.
