...
**Steps 1–2 were superseded after the work was built.** Branching from
`scosman/eval-api-improvements` meant carrying PR #1517 (`leonard/kil-686-eval-job`), which is
draft, unmerged, and contains explicit merge blockers — none of which may reach a main-destined
branch. The project was rebuilt on `origin/scosman/evals_v2` directly as
`claude/eval-splits-evals-v2`, which makes step 2's merge unnecessary rather than resolved. The
cost is phase 5: its code targets #1517's eval job worker and does not come along. See
`implementation_plan.md`'s phase 1 and phase 5 entries. The original branch,
`claude/eval-splits-v1-v2-q38412`, is kept as the record.
