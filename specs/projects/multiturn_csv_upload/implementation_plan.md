...
- [x] **Phase 2 — Frontend dialog + sample asset.** Regenerate the OpenAPI client (`app/web_ui/src/lib/generate_schema.sh`). Update `upload_dataset_dialog.svelte` with the turn-mode-aware help block, dialog title, and sample-download link. Add `app/web_ui/static/sample_multiturn.csv`. Cover with the Vitest cases in `architecture.md` §6.3.
